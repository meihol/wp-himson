<?php
// Breadcrumbs
if ( ! function_exists( 'makaffo_get_breadcrumbs' ) ) {
    function makaffo_get_breadcrumbs ( 
        $list_style = 'ul', 
        $list_id = 'breadcrumbs', 
        ...
        // Front page
        if ( is_front_page() ) {
            $breadcrumb .= '<li class="' . $active_class . '">' . esc_html__('Home', 'makaffo') . '</li>'; //get_bloginfo( 'name' )
        } else {
            $breadcrumb .= '<li><a class="hover" href="' . home_url() . '">' . esc_html__('Home', 'makaffo') . '</a></li>'; //get_bloginfo( 'name' )
        }
        // Blog archive
        if ( 'page' == get_option( 'show_on_front' ) && get_option( 'page_for_posts' ) ) {
            ...
            if ( is_home() ) {
                ...
                $breadcrumb .= '<li><a class="hover" href="' . get_permalink( $blog_page_id ) . '">' . get_the_title( $blog_page_id )  . '</a></li>';
            }
        }
        ...
        if ( is_page() && ! is_front_page() ) {
            ...
            if ( $post->post_parent ) {
                ...
                while ( $parent_id ) {
                    ...
                    $crumbs[] = '<a class="hover" href="' . get_permalink( $page->ID ) . '">' . get_the_title( $page->ID ) . '</a>';
        ...
        if ( is_attachment() ) {
            ...
            $post = get_post( get_the_ID() );
            if ( $post->post_parent ) {
                $breadcrumb .= '<li><a class="hover" href="' . get_permalink( $post->post_parent ) . '">' . get_the_title( $post->post_parent ) . '</a></li>';
            }
            // Attachment title
            $breadcrumb .= '<li class="' . $active_class . '">' . get_the_title() . '</li>';
        }
        // Search
        if ( is_search() ) {
            $breadcrumb .= '<li class="' . $active_class . '">' . esc_html__( 'Search', 'makaffo' ) . '</li>';
        }
        // 404
        if ( is_404() ) {
            $breadcrumb .= '<li class="' . $active_class . '">' . esc_html__( '404', 'makaffo' ) . '</li>';
        }
        ...
        if ( is_tax() ) {
            ...
            foreach ( $attached_to as $post_type ) {
                ...
                if ( ! $output && get_post_type_archive_link( $cpt_obj->name ) ) {
                    $archive_link = '';
                    if ( 'ot_portfolio' == get_post_type() && makaffo_get_option('portfolio_archive') == 'archive_custom' ) {
                        $archive_link = get_page_link( makaffo_get_option('archive_page_custom') );                                
                    } else {
                        $archive_link = get_post_type_archive_link( $cpt_obj->name );
                    }
                    $breadcrumb .= '<li><a class="hover" href="' . $archive_link . '">' . $cpt_obj->labels->singular_name . '</a></li>';
                    $output = true;
                }
            }
            // Term title
                ...
                if ( $cpt_taxes && is_taxonomy_hierarchical( $cpt_taxes[0] ) ) {
                    ...
                    if ( is_array( $cpt_terms ) ) {
                        foreach( $cpt_terms as $cpt_term ) {
                            ...
                            $breadcrumb .= '<li><a class="hover" href="' . esc_url( $term_link ) . '">' . $cpt_term->name . '</a></li>';
                        }
                    }
                }
            }
...
if ( ! function_exists( 'makaffo_breadcrumbs' ) ) {
    function makaffo_breadcrumbs (){

        if( makaffo_get_option('breadcrumbs') && !is_front_page() ){ echo makaffo_get_breadcrumbs(); } ?>
